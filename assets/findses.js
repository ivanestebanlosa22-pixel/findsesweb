...
(function(){
  ...
  window.addEventListener('load',()=>{
    ...
    const scene=new THREE.Scene(); scene.fog=new THREE.FogExp2(0x060507,.05);
    const cam=new THREE.PerspectiveCamera(55,innerWidth/innerHeight,.1,100); cam.position.z=6;
    const cv=document.createElement('canvas');cv.width=cv.height=64;const g=cv.getContext('2d');
    const gr=g.createRadialGradient(32,32,0,32,32,32);gr.addColorStop(0,'#fff');gr.addColorStop(.4,'rgba(255,210,120,.6)');gr.addColorStop(1,'rgba(0,0,0,0)');
    ...
    const cols=[new THREE.Color(0xffcb45),new THREE.Color(0x8a5d0a),new THREE.Color(0x9c46ff)],t=new THREE.Color();
